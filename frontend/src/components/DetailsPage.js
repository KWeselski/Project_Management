...
class DetailsPage extends Component {
  ...
  getData = () => {
    ...
    axios.get(`/api/get_comments/${data.id}`).then((res) => {
  ...
  render() {
    ...
    const creator =
      profiles[profiles.findIndex((x) => x.user === data.creator)];
    return (
          ...
          <Paper variant="outlined" square style={{ marginTop: 20 }}>
            ...
            >
              <Typography variant="h6">
                Start Date: {this.getCurrentDate(new Date(data.start_date))}
              </Typography>
              <Typography variant="h6">
                End Date: {this.getCurrentDate(new Date(data.end_date))}
              </Typography>
            </Grid>
          </Paper>
        </Grid>
